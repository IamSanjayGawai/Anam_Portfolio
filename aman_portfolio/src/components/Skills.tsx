...
const Skills = ({ sectionRefs }: SkillsProps) => {
  ...
  return (
    <section
      ...
      className="section flex flex-col gap-12 items-center justify-center px-4 py-6"
    >
      <h2 className="text-3xl md:text-4xl font-extrabold glow-text mb-4">
        Security Expertise
      </h2>
      ...
      <div className="grid md:grid-cols-3 gap-8 w-full max-w-5xl">
        {skills.map((category, idx) => (
          <div
            ...
            className="glass rounded-2xl p-6 shadow-lg border border-cyan-500/20 hover:border-cyan-400/40 transition-all duration-300 group"
          >
            <div className="flex items-center gap-3 mb-4">
              <span className="text-cyan-400 text-2xl group-hover:scale-110 transition-transform duration-300">
                {category.icon}
              </span>
              <span className="font-bold text-lg text-cyan-300 group-hover:text-cyan-200 transition-colors">
                {category.title}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {category.items.map((skill, skillIdx) => (
                <div key={skillIdx} className="group/skill relative overflow-hidden">
                  <div className="bg-gradient-to-r from-slate-900/80 to-slate-800/60 rounded-lg p-4 border border-slate-700/40 hover:border-cyan-500/60 transition-all duration-300 backdrop-blur-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-200 font-medium group-hover/skill:text-cyan-200 transition-colors duration-300">
                        {skill.name}
                      </span>
                      <div className="flex space-x-1">
                        {[...Array(5)].map((_, i) => (
